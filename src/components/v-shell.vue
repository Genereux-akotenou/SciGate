<template>
    <div @click="$refs.cmd.focus();">
        ...
        <div id="input-line" class="input-line">
            ...
            <input v-model="value" ref="cmd" @keydown.enter="cmd_enter($event)" @keydown.up="history_up()" @keydown.down="history_down()" 
                @keydown.tab="cmd_tab($event)" class="cmdline" autofocus
            />
        </div>
        </div>
    </div>
    ...
    methods: {
      ...
      async cmd_enter() {
        ...
        if (cmd == "clear") {
          ...
          this.output(
            ...
            console.log('-->')
            const executeCommandResponse = await window.electron.sshExecuteCommand(cmd);
            if (executeCommandResponse.success) {
                console.log('Command output:', executeCommandResponse.result);
                this.output(executeCommandResponse.result);
            }
      ...
      output(html) {
        this.$refs.output.insertAdjacentHTML(
          "beforeEnd",
          "<pre style='text-align:left;'>" + html + "</pre>"
        );
        this.value = "";
      }
    },
